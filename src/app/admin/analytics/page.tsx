...
export default async function AnalyticsPage({ searchParams }: { searchParams: Promise<{ cursor?: string; jurisdiction?: string; from?: string; to?: string }> }) {
  ...
  const jurisdictionCode = parameters.jurisdiction?.trim().toUpperCase() || null;
  let failed = false;
  ...
  if (jurisdictionCode) next.set("jurisdiction", jurisdictionCode);
  ...
  return <div className="mx-auto max-w-[90rem]">
    ...
    <form className="mt-7 grid gap-3 border-b border-slate-400 pb-7 sm:grid-cols-2 lg:grid-cols-[9rem_11rem_11rem_auto] lg:items-end" method="get">
      <label className="grid gap-1 text-sm font-semibold">Jurisdiction<input name="jurisdiction" defaultValue={jurisdictionCode ?? ""} maxLength={2} placeholder="All" className="min-h-11 border border-slate-500 bg-white px-3 uppercase" /></label>
