...
beforeEach(() => {
  ...
  mocks.fetchAuthQuery.mockResolvedValue({
    page: [{ id: "metric-1", day: "2026-07-28", jurisdictionId: "jurisdiction-ghana", jurisdictionName: "Ghana", jurisdictionKind: "geographic", totalQuestions: 15, successCount: 14, failureCount: 1, abortedCount: 0, providerFailureCount: 1, p50UpperBoundMs: 500, p95UpperBoundMs: 1000, updatedAt: Date.now() }],
    isDone: false,
...
describe("admin analytics page", () => {
  it("requests only the bounded daily aggregate register and preserves filters in pagination", async () => {
    ...
    expect(screen.getByRole("region", { name: "Lazy analytics charts" })).toHaveAttribute("data-props", JSON.stringify({ jurisdictionId: "jurisdiction-ghana", fromDay: "2026-07-01", toDay: "2026-07-28" }));
