...
const mocks = vi.hoisted(() => ({ authorizeAdminPage: vi.fn(), fetchAuthQuery: vi.fn(), redirect: vi.fn(), refresh: vi.fn() }));
...
vi.mock("convex/react", () => ({ useMutation: () => vi.fn(), usePaginatedQuery: () => ({ results: [], status: "Exhausted", loadMore: vi.fn() }) }));
...
afterEach(cleanup);
describe("incidents register", () => {
  it("renders bounded severity and status filters", async () => { render(await IncidentsPage({ searchParams: Promise.resolve({ status: "investigating" }) })); expect(screen.getByRole("heading", { name: "Incidents" })).toBeVisible(); expect(screen.getByRole("table", { name: "System incidents" })).toBeVisible(); expect(screen.getByLabelText("Status")).toHaveValue("investigating"); });
  it("shows the create-incident form only to administrators with write permission", async () => {
    mocks.authorizeAdminPage.mockResolvedValue({ status: "authorized", currentAdmin: { userId: "root", roles: ["super_admin"] } });
    render(await IncidentsPage({ searchParams: Promise.resolve({}) }));
    expect(screen.getByRole("heading", { name: "Open incident" })).toBeVisible();
  });
});
