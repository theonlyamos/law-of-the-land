...
export default async function IncidentsPage({ searchParams }: { searchParams: Promise<AdminTableSearchParams> }) {
  ...
  return <div className="mx-auto max-w-[88rem]"><header className="border-b-2 border-slate-700 pb-7"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-800">Control · response docket</p><h1 className="mt-3 text-5xl font-semibold tracking-[-0.05em]">Incidents</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-slate-700">Owned operational investigations with immutable notes and status history.</p></header><div className="mt-8"><DataTable ariaLabel="System incidents" basePath="/admin/incidents" columns={[{ key: "incident", label: "Incident" }, { key: "severity", label: "Severity" }, { key: "status", label: "Status" }, { key: "owner", label: "Owner" }, { key: "details", label: "Details" }]} filters={[{ name: "status", label: "Status", value: status ?? "", options: [{ value: "", label: "All states" }, ...STATUSES.map((value) => ({ value, label: value }))] }, { name: "severity", label: "Severity", value: severity ?? "", options: [{ value: "", label: "All severities" }, ...SEVERITIES.map((value) => ({ value, label: value }))] }]} rows={result.page.map((row) => ({ id: row.id, cells: { incident: <span className="font-semibold">{row.title}</span>, severity: row.severity, status: row.status, owner: row.ownerId ?? "Unassigned", details: <IncidentActions incidentId={row.id as Id<"systemIncidents">} canWrite={canWrite} /> } }))} currentCursor={navigation.cursor} previousCursors={navigation.previousCursors} nextCursor={result.continueCursor} isDone={result.isDone} state={failed ? "error" : "ready"} /></div></div>;
}
