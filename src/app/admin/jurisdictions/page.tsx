  ...
  return <div className="mx-auto max-w-[88rem]">
    ...
    <div className="mt-8"><DataTable
      ...
      rows={rows.map((row) => ({ id: row.id, cells: {
        ...
        context: row.geographic ? <span className="grid gap-1"><strong>{title(row.geographic.level)}</strong><Link href={`/admin/jurisdictions/${row.id}/website-chat`} className="text-sm underline">Chat widget</Link>{canWrite && <details className="mt-3"><summary className="cursor-pointer text-sm underline">Manage chat widget allowance</summary><div className="mt-4 min-w-60"><WidgetAllowanceControl jurisdictionId={row.id} /></div></details>}<span className="text-xs">{row.geographic.parent ? `Within ${row.geographic.parent.name} (${title(row.geographic.parent.level)})` : "Root geography"}</span></span> : row.organization ? <span className="grid gap-1"><strong>{row.organization.name}</strong><OrganizationWidgetControls organizationId={row.organization.id} /><span className="text-xs">{title(row.organization.class)} / {title(row.organization.status)} / {title(row.scopeMode ?? "global")}</span></span> : <span>Awaiting typed migration</span>,
