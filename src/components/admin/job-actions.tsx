...
export function JobActions({ jobId, status, canRetry, canCancel }: { jobId: Id<"integrationJobs">; status: "queued" | "running" | "waiting_provider" | "succeeded" | "failed" | "cancelled" | "manual_review"; canRetry: boolean; canCancel: boolean }) {
  ...
  const execute = async (kind: "retry" | "cancel") => { setState("working"); try { const idempotencyKey = `${kind}_${crypto.randomUUID().replaceAll("-", "")}`; await (kind === "retry" ? retry : cancel)({ jobId, reason, idempotencyKey }); setState("done"); router.refresh(); } catch { setState("error"); } };
