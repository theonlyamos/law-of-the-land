  ...
  return (
    <div className="space-y-10">
      {[...grouped.entries()].map(([key, group]) => {
        const chronological = [...group.metrics].sort((left, right) => left.day.localeCompare(right.day));
        const maximum = Math.max(1, ...chronological.map((row) => row.totalQuestions));
        return (
          <section key={key} role="region" aria-label={group.label}>
            <h3 className="mb-3 text-sm font-semibold text-slate-800 [overflow-wrap:anywhere]">{group.label}</h3>
