...
const metrics = [
  { id: "metric-1", day: "2026-07-27", jurisdictionId: "jurisdiction-ghana", jurisdictionName: "Ghana", totalQuestions: 10, failureCount: 9, providerFailureCount: 2, p95UpperBoundMs: 2500 },
  { id: "metric-2", day: "2026-07-28", jurisdictionId: "jurisdiction-ghana", jurisdictionName: "Ghana", totalQuestions: 15, failureCount: 8, providerFailureCount: 1, p95UpperBoundMs: 1000 },
];

describe("analytics charts", () => {
  it("separates and labels stable-ID jurisdiction series", async () => {
    convexMocks.useQuery.mockImplementation((_reference, args) => args === "skip" ? undefined : {
      page: [
        { ...metrics[0], id: "stable-1" },
        { ...metrics[1], id: "stable-2" },
        { ...metrics[0], id: "other-id", jurisdictionId: "jurisdiction-other-ghana", jurisdictionName: "Ghana", totalQuestions: 7 },
      ],
      isDone: true,
      continueCursor: "",
    });
    render(<AnalyticsCharts jurisdictionId={null} fromDay="2026-07-01" toDay="2026-07-28" />);
    await act(async () => {
  ...
  it("does not import or render chart graphics until the section enters view", async () => {
    render(<AnalyticsCharts jurisdictionId="jurisdiction-ghana" fromDay="2026-07-01" toDay="2026-07-28" />);
    ...
    expect(convexMocks.useQuery.mock.calls.at(-1)![1]).toEqual({ paginationOpts: { numItems: 50, cursor: null }, jurisdictionId: "jurisdiction-ghana", fromDay: "2026-07-01", toDay: "2026-07-28" });
  });
  ...
  it("uses labels and line patterns so status is not communicated by color alone", async () => {
    ...
    expect(screen.getByText("15 questions, 1 provider failure, p95 ≤ 1,000 ms")).toBeVisible();
  });
});
