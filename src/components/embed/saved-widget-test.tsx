...
export function SavedWidgetTest({ publicId, onClose }: { publicId: string; onClose: () => void }) {
  ...
  return <div className="space-y-3"><p className="text-sm">This tests saved settings and uses your organization's allowance.</p>{!ready && <p role="status">Loading saved widget. If it doesn't load, check that website chat is enabled and your library is ready.</p>}<iframe ref={frame} title="Test saved website chat" className="h-[640px] w-full rounded-lg border" referrerPolicy="no-referrer" sandbox="allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox" src={`/embed/${encodeURIComponent(publicId)}?parentOrigin=${encodeURIComponent(identity.origin)}&instanceId=${identity.instanceId}`} /></div>;
}
