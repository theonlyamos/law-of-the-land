...
3. Disable the deployment gate on the exact target. For staging, use the approved staging reference shown here. For production, first complete the exact-name validation and typed confirmation in [bootstrap](bootstrap.md#2-bind-every-bootstrap-command-to-the-exact-production-name), then substitute `$ApprovedProductionDeployment` for `staging`; never use the `prod` alias.
...
5. Rotate a Better Auth/provider secret only when exposure is credible, then invalidate affected sessions. For an indexing-operation disclosure, wait for the known provider outcome; a retried provider job receives a fresh lease after claim.
...
Do not delete jurisdictions, documents, audit events, jobs, or Gemini File Search store bindings. Recover by fixing preview, rerunning the release checklist, enabling the persisted row with exact `ADMIN_PANEL <ADMIN_ENVIRONMENT> ENABLE`, and only then setting the deployment gate true. During an explicitly approved production incident, use `--deployment $ApprovedProductionDeployment` only after the exact-name validation and typed confirmation in [bootstrap](bootstrap.md#2-bind-every-bootstrap-command-to-the-exact-production-name); never substitute the `prod` alias.
