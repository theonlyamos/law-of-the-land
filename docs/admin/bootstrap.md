...
2. In the Vercel **Production** environment, configure the Vercel-owned variables in the matrix. `GROUNDX_API_KEY`, `GOOGLE_AI_API_KEY`, `SEARCH_JURISDICTION_SECRET`, and `TELEMETRY_INGEST_SECRET` are server-only even though Vercel also needs them; never prefix them with `NEXT_PUBLIC_`. The shared search and telemetry secrets must each match Convex exactly.
