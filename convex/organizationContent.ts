...
import {
  createResourceArgs,
  createResourceForActor,
  updateResourceArgs,
  updateResourceForActor,
  archiveResourceArgs,
  archiveResourceForActor,
  markResourceRepealedArgs,
  markResourceRepealedForActor,
} from "./admin/resources";
import {
  createDocumentVersionArgs,
  createDocumentVersionForActor,
  uploadLimit,
...
export const approveVersion = mutation({
  ...
  handler: async (ctx, args) =>
    decideForActor(
      ...
      args,
      "approve",
      await reviewerForVersion(ctx, args.versionId),
    ),
});
...
export const listResources = query({
  ...
  handler: async (ctx, args) => {
    ...
    const jurisdiction = args.jurisdictionId
      ? (
          await requireOrganizationJurisdiction(
            ctx,
            args.organizationId,
            args.jurisdictionId,
            "read",
      ...
      : await singleOrganizationJurisdiction(ctx, args.organizationId);
    if (!jurisdiction) return { page: [], isDone: true, continueCursor: "" };
    return ctx.db
      .query("legalResources")
      .withIndex("by_jurisdictionId_and_updatedAt", (q) =>
...
export const getResource = query({
  ...
  handler: async (ctx, args) => {
    const { resource, actor, jurisdiction } = await requireOrganizationResource(
      ctx,
      args.resourceId,
      "read",
    );
    if (
      ...
      (args.jurisdictionId && resource.jurisdictionId !== args.jurisdictionId)
    )
      throw new ConvexError("ORGANIZATION_ACCESS_DENIED");
    return {
      resource,
      role: actor.organizationRole,
      canReview: actor.canReview && jurisdiction.status !== "archived",
      canManage: actor.canManage && jurisdiction.status !== "archived",
      isOwner: actor.isOwner,
      maxBytes: Math.min(uploadLimit(), 4 * 1024 * 1024),
    };
  },
});
export const listVersions = query({
  ...
  handler: async (ctx, args) => {
    const { actor, resource } = await requireOrganizationResource(
      ctx,
      args.resourceId,
      "read",
    );
        ...
        result.page.map(async (row) => {
              ...
              ? ctx.db
                  ...
                  .withIndex("by_resourceId_and_versionNumber", (q) =>
                    q
                      ...
                      .eq("versionNumber", row.versionNumber - 1),
                  )
                  .unique()
              : null,
            ctx.db
              .query("reviewDecisions")
              .withIndex("by_documentVersionId_and_createdAt", (q) =>
                q.eq("documentVersionId", row._id),
              )
              .order("desc")
              .take(20),
          ...
          return {
            ...
            decisions: decisions.map((d) => ({
              ...
              reason: d.reason,
              evaluationRunId: d.evaluationRunId,
...
export const prepareUpload = mutation({
  args: { resourceId: v.id("legalResources") },
  handler: async (ctx, args) => {
    const { actor, resource } = await requireOrganizationResource(
      ctx,
      args.resourceId,
    ...
    return {
      ...
      maximumBytes: Math.min(uploadLimit(), 4 * 1024 * 1024),
...
export const finalizeUpload = mutation({
  args: {
    ...createDocumentVersionArgs.fields,
    issuedAt: v.number(),
    signature: v.string(),
  },
  handler: async (ctx, args) => {
    ...
    if (
      !(await verifyWidgetServiceProof(
        "organization-upload-finalize",
        args.issuedAt,
        uploadProofBytes(actor, args),
        args.signature,
      ))
    )
      throw new ConvexError("UPLOAD_PROOF_INVALID");
    const attached = await ctx.db
      .query("documentVersions")
      .withIndex("by_originalStorageId", (q) =>
        q.eq("originalStorageId", args.storageId),
      )
      .take(2);
    if (attached.length) {
      if (
        attached.length === 1 &&
        attached[0].resourceId === args.resourceId &&
        attached[0].sha256 === args.sha256 &&
        attached[0].submittedBy === actor.userId
      )
        return attached[0]._id;
...
export const retryPublication = mutation({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    if (
      !job ||
      !versionId ||
      !["gemini_index_document", "gemini_delete_document"].includes(job.type)
    )
      throw new ConvexError("ORGANIZATION_ACCESS_DENIED");
    ...
    if (
      ...
      (job.type === "gemini_index_document" &&
        (job.recoveryKind !== "poll_operation" || !job.providerOperationName))
    )
      throw new ConvexError("JOB_NOT_SAFELY_RETRYABLE");
...
export const publicationStatus = query({
  ...
  handler: async (ctx, args) => {
    ...
    const lock = await ctx.db
      ...
      .withIndex("by_resourceId", (q) => q.eq("resourceId", version.resourceId))
      .unique();
    const job =
      lock?.versionId === args.versionId && lock.jobId
        ? await ctx.db.get(lock.jobId)
        : null;
    return job
      ? {
          id: job._id,
