...
it("resolves only the selected public organization and denies foreign frames and disabled widgets", async () => {
...
it("rejects disabled, untyped, or misowned geographical libraries", async () => {
  ...
  for (const patch of [{ status: "draft" as const }, { status: "archived" as const }, { kind: undefined }, { organizationId: f.organizationId }]) {
